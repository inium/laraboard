...
Laravel 인증 스캐폴딩의 RegisterController에 개발자가 게시판 사용자 등록 Trait을 추가하여 회원가입과 동시에 게시판 사용자 등록을 할 수 있습니다. Laravel의 회원가입 스캐폴딩은 별도로 닉네임을 입력하지 않기 때문에 본 게시판 패키지에서는 사용자 이름을 사용하도록 하였으며 중복을 허용하지 않기 때문에 5자리 해시코드를 추가하여 {사용자이름_해시코드5자리} 형태로 생성합니다.
